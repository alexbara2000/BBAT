...
function copyStackTrace() {
  return async ({
    ...
    const frames = (0, _selectors.getCurrentThreadFrames)(getState());
    const shouldDisplayOriginalLocation = (0, _selectors.getShouldSelectOriginalLocation)(getState());
    const framesToCopy = frames.map(frame => (0, _frames.formatCopyName)(frame, L10N, shouldDisplayOriginalLocation)).join("\n");
...
function showFrameContextMenu(event, frame, hideRestart = false) {
  return async ({
    ...
    if (!hideRestart && isValidRestartFrame(frame)) {
      items.push(formatMenuElement("restartFrame", () => dispatch((0, _pause.restart)(frame))));
    }

    const toggleFrameWorkL10nLabel = (0, _selectors.getFrameworkGroupingState)(getState()) ? "framework.disableGrouping" : "framework.enableGrouping";
    items.push(formatMenuElement(toggleFrameWorkL10nLabel, () => dispatch((0, _ui.toggleFrameworkGrouping)(!(0, _selectors.getFrameworkGroupingState)(getState())))));
    const {
      source
    } = frame;
    ...
    if (frame.source) {
      ...
      items.push(formatMenuElement(toggleBlackBoxL10nLabel, () => dispatch((0, _pause.toggleBlackBox)(source))));
    }
