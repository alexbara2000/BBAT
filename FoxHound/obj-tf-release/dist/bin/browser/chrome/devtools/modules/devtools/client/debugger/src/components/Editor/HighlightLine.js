...
function isDocumentReady(selectedLocation, selectedSourceTextContent) {
  return selectedLocation && selectedSourceTextContent && (0, _sourceDocuments.hasDocument)(selectedLocation.source.id);
}
...
class HighlightLine extends _react.Component {
  ...
  shouldSetHighlightLine(selectedLocation, selectedSourceTextContent) {
    ...
    const editorLine = (0, _editor.toEditorLine)(selectedLocation.source.id, line);
  ...
  resetHighlightLine(doc, editorLine) {
    ...
    setTimeout(() => doc && doc.removeLineClass(editorLine, "wrap", "highlight-line"), duration);
  }
