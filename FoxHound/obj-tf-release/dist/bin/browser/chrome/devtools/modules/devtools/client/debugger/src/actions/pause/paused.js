...
function paused(pauseInfo) {
  return async function ({
    ...
    await dispatch((0, _.fetchFrames)(thread)); // And map them to original source locations.
    ...
    await dispatch((0, _.mapFrames)(thread)); // If we paused on a particular frame, automatically select the related source
    // and highlight the paused line

    const selectedFrame = (0, _selectors.getSelectedFrame)(getState(), thread);

    if (selectedFrame) {
      await dispatch((0, _sources.selectLocation)(selectedFrame.location)); // We might have resumed while opening the location.
