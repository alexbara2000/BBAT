...
class DebugLine extends _react.PureComponent {
  ...
  setDebugLine(why, location) {
    ...
    const doc = (0, _editor.getDocument)(location.source.id);
    let {
      line,
      column
  ...
  clearDebugLine(why, location) {
    ...
    if (!location || !(0, _editor.hasDocument)(location.source.id)) {
      return;
    }
...
function isDocumentReady(location, sourceTextContent) {
  return location && sourceTextContent && (0, _editor.hasDocument)(location.source.id);
}
