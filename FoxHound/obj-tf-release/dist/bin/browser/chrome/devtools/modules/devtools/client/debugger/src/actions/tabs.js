...
function closeTabs(sources) {
  return ({
    ...
    dispatch((0, _sources.selectSource)(newSourceToSelect));
  };
}
/**
...
function getNewSourceToSelect(state, closedTabsSources) {
  const selectedLocation = (0, _selectors.getSelectedLocation)(state); // Do not try to select any source if none was selected before
  ...
  const tabsSources = (0, _selectors.getSourcesForTabs)(state); // Assume that `sources` is a consecutive list of tab's sources
