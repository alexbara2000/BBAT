...
function showEditorContextMenu(event, editor, location) {
  return async ({
    dispatch,
    getState
  }) => {
    ...
    const hasMappedLocation = (source.isOriginal || (0, _selectors.isSourceWithMap)(state, source.id) || (0, _source.isPretty)(source)) && !(0, _selectors.getPrettySource)(state, source.id);
    const isSourceOnIgnoreList = (0, _selectors.isSourceMapIgnoreListEnabled)(state) && (0, _selectors.isSourceOnSourceMapIgnoreList)(state, source);
    const editorWrappingEnabled = (0, _selectors.getEditorWrapping)(state);
...
function showEditorGutterContextMenu(event, editor, location, lineText) {
  return async ({
    dispatch,
    getState
  }) => {
    ...
    (0, _menu.showMenu)(event, [...(0, _editorBreakpoint.createBreakpointItems)(location, lineText, dispatch), {
      ...
      type: "separator"
    }, blackBoxLineMenuItem(source, editor, blackboxedRanges, isSourceOnIgnoreList, location.line, dispatch)]);
  };
} // Menu Items
...
const blackBoxLineMenuItem = (selectedSource, editor, blackboxedRanges, isSourceOnIgnoreList, // the clickedLine is passed when the context menu
...
const blackBoxLinesMenuItem = (selectedSource, editor, blackboxedRanges, isSourceOnIgnoreList, clickedLine = null, dispatch) => {
  const {
    codeMirror
  } = editor;
  const from = codeMirror.getCursor("from");
...
function editorMenuItems({
  ...
  const startLine = (0, _editor.toSourceLine)(source.id, editor.codeMirror.getCursor("from").line);
  const endLine = (0, _editor.toSourceLine)(source.id, editor.codeMirror.getCursor("to").line); // Find any blackbox ranges that exist for the selected lines
  ...
  if (!theWholeSourceIsBlackBoxed) {
    ...
    items.push(blackBoxSourceLinesMenuItem(source, editor, blackboxedRanges, isSourceOnIgnoreList, null, dispatch));
  }
