...
function syncPendingBreakpoint(source, pendingBreakpoint) {
  return async thunkArgs => {
    ...
    const generatedSourceId = source.isOriginal ? (0, _index.originalToGeneratedId)(source.id) : source.id;
    ...
    if (source == generatedSource && isPendingBreakpointWithSourceMap) {
      ...
      return dispatch((0, _.addBreakpoint)(sourceGeneratedLocation, pendingBreakpoint.options, pendingBreakpoint.disabled, () => !client.hasBreakpoint(breakpointServerLocation)));
    }
    ...
    if (!newGeneratedLocation) {
      ...
      if (isPendingBreakpointWithSourceMap) {
        dispatch((0, _.removeBreakpointAtGeneratedLocation)(sourceGeneratedLocation));
      }
    ...
    return dispatch((0, _.addBreakpoint)(newGeneratedLocation, pendingBreakpoint.options, pendingBreakpoint.disabled));
  };
}