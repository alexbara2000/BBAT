  ...
  return async thunkArgs => {
    ...
    if (location && location.line >= 1 && (0, _source.getPrettySourceURL)(location.source.url) == prettySource.url) {
      ...
      if (location.source == prettySource) {
        return dispatch((0, _sources.selectSpecificLocation)(location));
      }
    }
...
async function prettyPrintSource(source, thunkArgs) {
  ...
  await dispatch((0, _pause.mapFrames)(sourceActor.thread)); // Update breakpoints locations to the new pretty/original source

  await dispatch((0, _breakpoints.updateBreakpointsForNewPrettyPrintedSource)(source)); // A mutated flag, only meant to be used within this module
...
const memoizedPrettyPrintSource = (0, _memoizableAction.memoizeableAction)("setSymbols", {
  getValue: (source, {
    ...
    const url = getPrettyOriginalSourceURL(source);
    const id = (0, _index.generatedToOriginalId)(source.id, url);
    const s = (0, _selectors.getSourceFromId)(getState(), id); // Avoid returning it if doTogglePrettyPrint isn't completed.
