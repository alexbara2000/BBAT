...
function toggleMapScopes() {
  return async function ({
    ...
    if ((0, _selectors.getOriginalFrameScope)(getState(), selectedFrame)) {
      return;
    } // Also ignore the call if we didn't fetch the scopes for the selected frame


    const scopes = (0, _selectors.getGeneratedFrameScope)(getState(), selectedFrame);
...
function getMappedScopes(scopes, locations) {
  return async function (thunkArgs) {
    ...
    if (!(0, _selectors.isMapScopesEnabled)(getState()) || !source || !generatedSource || generatedSource.isWasm || source.isPrettyPrinted || !source.isOriginal) {
      return null;
    ...
    try {
      ...
      (0, _selectors.getSettledSourceTextContent)(getState(), locations.location);
