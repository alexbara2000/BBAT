...
function validateSelectedFrame(state, selectedFrame) {
  const newThread = (0, _selectors.getCurrentThread)(state);

  if (selectedFrame.thread != newThread) {
    throw new ContextError("Selected thread has changed");
  }

  const newSelectedFrame = (0, _selectors.getSelectedFrame)(state, newThread); // Compare frame's IDs as frame objects are cloned during mapping

  if (selectedFrame.id != (newSelectedFrame === null || newSelectedFrame === void 0 ? void 0 : newSelectedFrame.id)) {
...
function validateBreakpoint(state, breakpoint) {
  ...
  if (!(0, _selectors.hasSource)(state, breakpoint.location.source.id)) {
    throw new ContextError(`Breakpoint's location is obsolete (source '${breakpoint.location.source.id}' no longer exists)`);
  }

  if (!(0, _selectors.hasSource)(state, breakpoint.generatedLocation.source.id)) {
...
function validateThreadFrames(state, thread, frames) {
  ...
  const newTopFrame = (0, _selectors.getCurrentlyFetchedTopFrame)(state, newThread);

  if ((newTopFrame === null || newTopFrame === void 0 ? void 0 : newTopFrame.id) != frames[0].id) {
    throw new ContextError("Thread moved to another location");
  }
}

function validateFrame(state, frame) {
  if (!(0, _selectors.hasFrame)(state, frame)) {
