    ...
    if (!blackboxedRangesForSelectedSource.length) {
      ...
      editor.codeMirror.operation(() => {
        blackboxedRangesForSelectedSource.forEach(range => {
          const start = (0, _editor.toEditorLine)(selectedSource.id, range.start.line); // CodeMirror.eachLine doesn't include `end` line offset, so bump by one

          const end = (0, _editor.toEditorLine)(selectedSource.id, range.end.line) + 1;
