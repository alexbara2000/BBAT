  ...
  return async function ({
    ...
    try {
      const sourceList = await Promise.all(sources.map(async sourceActor => {
        const originalSourcesInfo = await dispatch(loadSourceMap(sourceActor));
...
function checkSelectedSource(sourceId) {
  return async ({
    ...
    if (rawPendingUrl === source.url) {
      ...
      await dispatch((0, _sources.selectLocation)((0, _location.createLocation)({
        source,
...
function checkPendingBreakpoints(source, sourceActor) {
  return async ({
    ...
    await dispatch((0, _sources.setBreakableLines)((0, _location.createLocation)({
      source,
      sourceActor
    })));
    await Promise.all(pendingBreakpoints.map(pendingBp => {
      return dispatch((0, _breakpoints.syncPendingBreakpoint)(source, pendingBp));
    }));
  };
}
...
function newGeneratedSources(sourceResources) {
  return async ({
    ...
    (async () => {
      await dispatch(loadSourceMaps(newSourceActors)); // We would like to sync breakpoints after we are done
