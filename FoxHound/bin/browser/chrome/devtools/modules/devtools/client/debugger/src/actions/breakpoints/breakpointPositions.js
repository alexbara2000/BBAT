...
async function _setBreakpointPositions(location, thunkArgs) {
  ...
  if (location.source.isOriginal) {
    ...
    for (const range of ranges) {
      ...
      const allActorsPositions = await Promise.all((0, _selectors.getSourceActorsForSource)(getState(), generatedSourceId).map(actor => client.getSourceActorBreakpointPositions(actor, range))); // `allActorsPositions` looks like this:
      // [
    ...
    const allActorsBreakableColumns = await Promise.all((0, _selectors.getSourceActorsForSource)(getState(), location.source.id).map(async actor => {
...
function generatedSourceActorKey(state, source) {
  const generatedSource = (0, _selectors.getSource)(state, source.isOriginal ? (0, _index.originalToGeneratedId)(source.id) : source.id);
...
const setBreakpointPositions = (0, _memoizableAction.memoizeableAction)("setBreakpointPositions", {
  getValue: (location, {
    ...
    const positions = (0, _selectors.getBreakpointPositionsForSource)(getState(), location.source.id);
