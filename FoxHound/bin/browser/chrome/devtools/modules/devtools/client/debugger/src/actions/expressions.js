...
function addExpression(input) {
  return async ({
    ...
    const expressionError = await parserWorker.hasSyntaxError(input); // If the expression already exists, only update its evaluation

    let expression = (0, _selectors.getExpression)(getState(), input);
    ...
    if (!expression) {
      ...
      expression = (0, _selectors.getExpression)(getState(), input); // When there is an expression error, we won't store the expression
...
function autocomplete(input, cursor) {
  return async ({
    ...
    const result = await client.autocomplete(input, cursor, selectedFrame === null || selectedFrame === void 0 ? void 0 : selectedFrame.id); // Pass both selectedFrame and thread in case selectedFrame is null
...
function evaluateExpressions(selectedFrame) {
  return async function ({
    ...
    const thread = (selectedFrame === null || selectedFrame === void 0 ? void 0 : selectedFrame.thread) || (0, _selectors.getCurrentThread)(getState());
    const results = await client.evaluateExpressions(inputs, {
      ...
      frameId: selectedFrame === null || selectedFrame === void 0 ? void 0 : selectedFrame.id,
      threadId: thread
...
function evaluateExpression(expression) {
  return async function (thunkArgs) {
    ...
    const thread = (0, _selectors.getCurrentThread)(getState());
    const selectedFrame = (0, _selectors.getSelectedFrame)(getState(), thread);
    const selectedSource = (0, _selectors.getSelectedSource)(getState()); // Only map when we are paused and if the currently selected source is original,
