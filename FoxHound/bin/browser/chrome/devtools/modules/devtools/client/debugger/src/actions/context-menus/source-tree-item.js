...
function showSourceTreeItemContextMenu(event, item, depth, setExpanded, itemName) {
  return async ({
    ...
    const isOverridden = (0, _selectors.isSourceOverridden)(state, item.source);
    const isSourceOnIgnoreList = (0, _selectors.isSourceMapIgnoreListEnabled)(state) && (0, _selectors.isSourceOnSourceMapIgnoreList)(state, item.source);
    const projectRoot = (0, _selectors.getProjectDirectoryRoot)(state);
...
async function handleLocalOverride(dispatch, source, isOverridden) {
  if (!isOverridden) {
    ...
    if (localPath) {
      dispatch((0, _sources.setOverrideSource)(source, localPath));
    }
  } else {
    dispatch((0, _sources.removeOverrideSource)(source));
  }
}
...
function getBlackBoxSourcesGroups(state, item) {
  ...
  const blackBoxRanges = (0, _selectors.getBlackBoxRanges)(state);
